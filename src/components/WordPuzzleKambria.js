...
import kambriaWords from "../data/kambriaWords.json";
...
const WordPuzzleKambria = () => {
  const wordIndex = Math.floor(Math.random() * kambriaWords.length);
  const [word, setWord] = useState(kambriaWords[wordIndex].word);
  const [clue, setClue] = useState(kambriaWords[wordIndex].clue);
  ...
  const restartGame = () => {
    ...
    const randomWord =
      kambriaWords[Math.floor(Math.random() * kambriaWords.length)];
...
export default WordPuzzleKambria;
